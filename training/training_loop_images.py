...
# Bring utils packages onto the path
import sys, os
...
def training_loop(
    ...
    assert dataset_obj.resolution[1] % 2 ** (len(cfg.network.channel_mult) - 1) == 0
    ...
    if dist.get_rank() == 0:
        with torch.no_grad():
            images = torch.zeros([batch_gpu, net.img_channels] + net.img_resolution, device=device)
    ...
    while True:
        ...
        for round_idx in range(num_accumulation_rounds):
            ...
            with ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                images = next(dataset_iterator) # This line is normally written as : images, labels = next(dataset_iterator) 
                # However since our model is unconditional, in order to save GPU memory our Dataset object output only images without labels
                images = images.to(device).to(torch.float32)
                # labels = labels.to(device) // Original code 
